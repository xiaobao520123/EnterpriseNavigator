...
class TextProcess(object):
    def __init__(self, chain_dict_file):
        self.chain_dict_file = chain_dict_file
        chain_dict = {}
        index = 1
        with open(self.chain_dict_file, mode='r',encoding='utf-8') as f:
            while True:
                line = f.readline()
                if line is None or line is '':
                    break
                words = line.strip().lower()
                chain_dict[words] = index
                index = index+1
            f.close()
    ...
    def find_enterprise(self, text_objects):
        ...
        for text_obj in text_objects:
            ...
            if words.lower() in self.chain_dict: # 检查字段是否在品牌字典内，如果有则直接输出，结束过程
