...
from nlp.text_process import TextProcess
...
def init_model():
    ...
    shop_sign_chain_dict = cfg.shop_sign_chain_dict

    east_detect = TextDetection(detection_h5)
    recognition = TextRecognition(recognition_pb, label_txt, seq_len=cfg.seq_len)

    nlp = TextProcess(shop_sign_chain_dict)
    return east_detect, recognition, nlp

east_detect_model, recognition_model, nlp = init_model()
...
def find_enterprise_from_image(img_path):
    ...
    if not detection_text or len(detection_text) == 0:
        return None, None
    # 从文字信息中过滤出企业实体
    enterpise, polygon = nlp.find_enterprise(detection_text)
